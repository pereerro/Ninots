...
for (var i=0; i < Ninots.camins_de_ninots.length; i++) {
    ...
    obj.dibuixa = function(coords_svg, svg, classes) {
        return Ninots.dibuixa_ninots(coords_svg, svg, this.tipus, classes);
    };
...
Ninots.prototype.to_coords_pista = function(coords) {
    return [(coords[0] - this.marge_esq) / this.scalax, coords[1] / this.scalay];
}
...
Ninots.prototype.to_coords_svg = function(coords) {
    return [coords[0] * this.scalax + this.marge_esq, coords[1] * this.scalay];
}

Ninots.prototype.get_terreny_dims = function() {
    var ret = this.get_espai_ninos();
    ret.width -= this.marge*2*this.scalax;
    ret.height -= this.marge*2*this.scalay;
    ret.x += this.marge*this.scalax;
    ret.y += this.marge*this.scalay;
    return ret;
}

Ninots.prototype.get_espai_ninos = function() {
    return {width: this.svg_ninots.width.baseVal.value-this.marge_esq, height: this.svg_ninots.height.baseVal.value, x:this.marge_esq, y:0,};
}
...
Ninots.prototype.get_coords_cistelles = function() {
    ...
    return {cistella1: [dims.x + this.despl_cist * this.scalax, dims.y + dims.height / 2],
            cistella2: [dims.x + dims.width - this.despl_cist * this.scalax, dims.y + dims.height / 2]};
}
...
Ninots.prototype.resize = function() {
    ...
    this.camp.style.transform = 'scale('+this.scalax+','+this.scalay+') translate('+(-bb.x + this.marge + this.marge_esq / this.scalax)+'px,'+(-bb.y + this.marge)+'px)';
...
Ninots.prototype.dibuixa_camp = function() {
    ...
    camp.setAttribute('vector-effect','non-scaling-stroke');
...
Ninots.Ninot = function(obj_ninots, p_id, tipus, coords, despls, passi_a) {
    ...
    this.amb_pilota = tmpl.amb_pilota;
    this.pot_rebre = tmpl.pot_rebre;
    this.tmpl = tmpl;
...
Ninots.Ninot.prototype.dibuixa_fletxa = function() {
    ...
    if (this.ninot !== undefined)
        this.ninot.remove();
    if (this.fletxa !== undefined)
        this.fletxa.remove();
    ...
    this.fletxa_scala = [this.obj_ninots.scalax, this.obj_ninots.scalay];
    svg.appendChild(this.fletxa);

    // ninot after arrow by z-order
    this.ninot = Ninots.dibuixa_ninots(coords_svg, this.obj_ninots.svg_ninots, this.tipus);
...
Ninots.Ninot.prototype.dibuixa_passi = function() {
    ...
    [inici_passi, fi_passi] = Ninots.get_segment(inici_passi, fi_passi, 20);
...
Ninots.Ninot.prototype.has_ball_end = function() {
    if (this.amb_pilota && this.passi_a === undefined)
        return true;
    var ll = this.obj_ninots.ninots.llista;
    for (k in ll) {
        if (ll[k].passi_a === this)
            return true;
    }
    return false;
}
...
/**
* Reorienta el ninot segons el seu desplaçament
*/
Ninots.Ninot.prototype.rotate = function() {
    if (typeof this.fletxa === SVGPathElement) return;
    var p = this.get_coords_svg();
    var t = this.fletxa.getTotalLength();
    if (! t > 0) return;
    var p1 = this.fletxa.getPointAtLength(0);
    var p2 = this.fletxa.getPointAtLength(t / 10);
    var r = Ninots.angle_entre_punts([p1.x, p1.y], [p2.x, p2.y]);
    var tf = 'rotate('+(r+90)+','+p[0]+','+p[1]+')';
    this.ninot.setAttribute('transform', tf);
    return r;
}
